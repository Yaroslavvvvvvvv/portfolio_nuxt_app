...
import { isRateLimited, registerHit, resetRateLimit } from '../server/utils/rateLimit'
...
describe('rateLimit', () => {
  ...
  it('reset clears the counter', () => {
    ...
    for (let i = 0; i < 10; i++) registerHit(key, 60_000)
    expect(isRateLimited(key, 5, 60_000)).toBe(true)
    resetRateLimit(key)
    expect(isRateLimited(key, 5, 60_000)).toBe(false)
  })
  ...
  it('keeps separate counters per key', () => {
    ...
    for (let i = 0; i < 6; i++) registerHit('test:a', 60_000)
